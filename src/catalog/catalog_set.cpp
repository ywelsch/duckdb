...
bool CatalogSet::CommittedAfterStarting(CatalogTransaction transaction, transaction_t timestamp) {
	// The entry is committed but not part of this transaction's snapshot, so it is not our source of truth.
	// Bootstrap entries are stamped with the system transaction's timestamp and precede everything.
	return IsCommitted(timestamp) && !VisibleToSnapshot(timestamp, transaction.start_time) &&
	       timestamp > SYSTEM_TRANSACTION_TIMESTAMP;
}
