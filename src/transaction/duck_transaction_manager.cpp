...
DuckTransactionManager::GetCheckpointType(DuckTransaction &transaction, const UndoBufferProperties &undo_properties) {
	...
	if (has_other_transactions) {
		if (undo_properties.has_dropped_entries) {
			// if we have dropped catalog entries in this transaction we cannot checkpoint in the presence of other
			// transactions - these might still need the entries from BEFORE this transaction
		...
		// otherwise - we need to do a concurrent checkpoint (updates are fine: the update segments keep the older
		// versions for the other transactions)
...
ErrorData DuckTransactionManager::CommitTransaction(ClientContext &context, Transaction &transaction_p) {
	...
	if (checkpoint_decision.can_checkpoint && undo_properties.has_dropped_entries &&
