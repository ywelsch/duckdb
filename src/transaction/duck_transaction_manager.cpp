...
Transaction &DuckTransactionManager::StartTransaction(ClientContext &context) {
	...
	// snapshots must not observe published commits that are not yet durable, and must not
	// present a catalog version newer than what they observe (see UnsyncedCommit)
	auto snapshot_bound = GetSnapshotBound();
	start_time = MinValue<transaction_t>(start_time, snapshot_bound.start_time);
	...
	auto transaction = make_uniq<DuckTransaction>(*this, context, start_time, transaction_id, catalog_version);
...
DuckTransactionManager::GetCheckpointType(DuckTransaction &transaction, const UndoBufferProperties &undo_properties) {
	...
	if (has_other_transactions) {
		if (undo_properties.has_updates || undo_properties.has_dropped_entries) {
			// other transactions - or snapshots bounded below not-yet-durable commits - might
			// need data from before this transaction: we cannot checkpoint updates or drops
...
void DuckTransactionManager::Checkpoint(ClientContext &context, bool force) {
	...
	if (GetLastCommit() >= LowestActiveStart()) {
		// we cannot do a full checkpoint if any transaction needs to read old data - a bounded
		// snapshot can start exactly at the last commit id, which it does not see
...
DuckTransactionManager::SnapshotBound DuckTransactionManager::GetSnapshotBound() {
	...
	bound.start_time = durable_commit_bound + 1;
	// the oldest pending commit is the first one a bounded snapshot cannot observe
	bound.catalog_version = unsynced_commits.front().catalog_version;
	return bound;
}
...
DuckTransactionManager::UpdateTransactionHorizon(optional_ptr<DuckTransaction> exclude) {
	...
	// commits pending durability pin the horizon: bounded snapshots may still need their
	// pre-commit version information
...
ErrorData DuckTransactionManager::CommitTransaction(ClientContext &context, Transaction &transaction_p) {
	...
	if (!error.HasError()) {
		if (HasOtherTransactions(transaction) || wal_written) {
			// a WAL-written commit is only durable after the sync below: snapshots bounded
			// below it can still start and need its pre-commit state (e.g. deleted_rows_in_use)
	...
	if (commit_registered) {
		// make the commit durable before acknowledging it (group sync: one fsync can cover
		// many commits). The WAL pointer stays valid without the WAL lock: checkpoints only
		// destroy the WAL after draining, and our entry is only removed below (see UnsyncedCommit).
		...
		try {
			...
			if (FinishCommitDurability(info.commit_id, info.wal_sync_offset)) {
				// the bound advanced: sweep transactions it was pinning (they hold e.g. the
	...
	if (checkpoint_decision.can_checkpoint && (undo_properties.has_updates || undo_properties.has_dropped_entries) &&
		...
		// a snapshot bounded below this commit started during the sync window above and still
		// needs the pre-commit state: checkpointing updates/drops now is what GetCheckpointType
		// refuses - skip the checkpoint (impossible in the skip-wal case: it holds the WAL lock
		// and registers nothing, so no bounded snapshot can exist here)
