...
void RowGroupCollection::Checkpoint(TableDataWriter &writer, TableStatistics &global_stats) {
	...
	for (idx_t segment_idx = 0; segment_idx < segments.size(); segment_idx++) {
		...
		bool has_extra_has_metadata_blocks = pointer.has_metadata_blocks;
		...
		{
			if (dynamic_cast<SingleFileTableDataWriter *>(&checkpoint_state.writer) != nullptr) {
				...
				if (has_extra_has_metadata_blocks) {
					if (all_written_block_ids != all_read_block_ids) {
						throw InternalException("Reloading blocks just written does not yield same blocks: " +
						                        oss.str());
					}
				} else {
					if (!metadata_reuse) {
					...
					for (auto &block : all_written_block_ids) {
						if (all_read_block_ids.find(block) == all_read_block_ids.end()) {
							throw InternalException(
							    "Reloading blocks just written does not yield superset of blocks: " + oss.str());
						}
					}
				}
